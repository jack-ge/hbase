...
public interface HRegionInterface extends VersionedProtocol, Stoppable,
		Abortable {
	/**
	 ...
	 * @throws IOException
	 *             This can manifest as an Hadoop ipc {@link RemoteException}
	 */
	 ...
	 * one that immediately preceeds it.
	 * 
	 * @param regionName
	 *            region name
	 * @param row
	 ...
	 * @return The number of processed put's. Returns -1 if all Puts processed
	 *         successfully.
	 * @throws IOException
	 *             e
	 */
	 ...
	 * expectedValue. If it does, it adds the put. If passed expected value is
	 * null, then the check is for non-existance of the row/column.
	 * 
	 * @param regionName
	 *            region name
	 * @param row
	...
	/**
	 * Atomically checks if a row/family/qualifier value match the
	 * expectedValue. If it does, it adds the delete. If passed expected value
	 * is null, then the check is for non-existance of the row/column.
	 * 
	 * @param regionName
	 *            region name
	 * @param row
	 ...
	 * long-like, this could throw an exception. If passed expected value is
	 * null, then the check is for non-existance of the row/column.
	 * 
	 * @param regionName
	 *            region name
	...
	/**
	 * Appends values to one or more columns values in a row. Optionally Returns
	 * the updated keys after the append.
	 * <p>
	 * This operation does not appear atomic to readers. Appends are done under
	 * a row lock but readers do not take row locks.
	 * 
	 * @param regionName
	 *            region name
	 * @param append
	...
	/**
	 * Increments one or more columns values in a row. Returns the updated keys
	 * after the increment.
	 * <p>
	 ...
	 * @return Array of Results (map of values); array is empty if done with
	 *         this region and null if we are NOT to go to the next region
	 *         (happens when a filter rules that the scan is done).
	 * @throws IOException
	 *             e
	 */
	...
	/**
	 * Method used for doing multiple actions(Deletes, Gets and Puts) in one
	 * call
	 * 
	 * @param multi
	 * @return MultiResult
	 ...
	 * 
	 * @param familyPaths
	 *            List of (family, hfile path) pairs
	 * @param regionName
	 *            name of region to load hfiles into
	 * @return true if successful, false if failed recoverably
	 * @throws IOException
	 *             if fails unrecoverably
	 */
	 ...
	 * @param region
	 *            region to open
	 * @return RegionOpeningState OPENED - if region open request was
	 *         successful. ALREADY_OPENED - if the region was already opened.
	 *         FAILED_OPENING - if region opening failed.
	 * 
	 * @throws IOException
	 */
	public RegionOpeningState openRegion(final HRegionInfo region)
			throws IOException;
	 ...
	 * @throws IOException
	 */
	public boolean closeRegion(final HRegionInfo region) throws IOException;
	...
	/**
	 * Closes the region in the RS with the specified encoded regionName and
	 * will use or not use ZK during the close according to the specified flag.
	 * Note that the encoded region name is in byte format.
	 * 
	 ...
	 * <p>
	 * This method currently flushes the region and then forces a compaction
	 * which will then trigger a split. The flush is done synchronously but the
	...
	/**
	 * Compacts a column-family within a specified region. Performs a major
	 * compaction if specified.
	 * <p>
	 * This method is asynchronous.
	 * 
	 * @param regionInfo
	 ...
	 *            true to force major compaction
	 * @param columnFamily
	 *            column family within a region to compact
	 * @throws NotServingRegionException
	 * @throws IOException
	 */
	void compactRegion(HRegionInfo regionInfo, boolean major,
			byte[] columnFamily) throws NotServingRegionException, IOException;

	/**
	 * Replicates the given entries. The guarantee is that the given entries
	 * will be durable on the slave cluster if this method returns without any
	 * exception. hbase.replication has to be set to true for this to work.
	 * 
	 * @param entries
	...
	public abstract List<Long> getCoprocessorProgress(List<String> paramList);
	 ...
	 * BlockCacheColumnFamilySummary objects. This method could be fairly
	 * heavyweight in that it evaluates the entire HBase file-system against
	 * what is in the RegionServer BlockCache.
	 * 
	 ...
	 *         we can clean logs. Returns null if nothing to flush. Names are
	 *         actual region names as returned by
