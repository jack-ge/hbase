 ...
 * Comparators are provided. Comparators change with context, whether user table
 * or a catalog table comparison. Its critical you use the appropriate
 * comparator. There are Comparators for KeyValue instances and then for just
 * the Key portion of a KeyValue used mostly by {@link HFile}.
 * 
 * <p>
 * KeyValue wraps a byte array and takes offsets and lengths into passed array
 * at where to start interpreting the content as KeyValue. The KeyValue format
 * inside a byte array is:
 * <code>&lt;keylength> &lt;valuelength> &lt;key> &lt;value></code> Key is
 * further decomposed as:
 * <code>&lt;rowlength> &lt;row> &lt;columnfamilylength> &lt;columnfamily> &lt;columnqualifier> &lt;timestamp> &lt;keytype></code>
 * The <code>rowlength</code> maximum is <code>Short.MAX_SIZE</code>, column
 * family length maximum is <code>Byte.MAX_SIZE</code>, and column qualifier +
 * key length must be < <code>Integer.MAX_SIZE</code>. The column does not
...
public class KeyValue implements Writable, HeapSize {
	...
	/**
	 * Comparator for plain key; i.e. non-catalog table key. Works on Key
	 * portion of KeyValue only.
	 */
	 ...
	 * 
	 * Hopefully we can get rid of this, I added this here because it's
	 * replacing something in HSK. We should move completely off of that.
	 * 
	 * @param tableName
	 *            The table name.
	...
	/**
	 * Key type. Has space for other key types to be added later. Cannot rely on
	...
	public static enum Type {
		...
		/**
		 * Cannot rely on enum ordinals . They change if item is removed or
		 * moved. Do our own codes.
		 * 
		 * @param b
	...
	/**
	 * Lowest possible key. Makes a Key with highest possible Timestamp, empty
	 * row and column. No key can be equal or lower than this one in memstore or
	 * in store file.
	 */
	public static final KeyValue LOWESTKEY = new KeyValue(
			HConstants.EMPTY_BYTE_ARRAY, HConstants.LATEST_TIMESTAMP);
	...
	/**
	 * Creates a KeyValue from the start of the specified byte array. Presumes
	 * <code>bytes</code> content is formatted as a KeyValue blob.
	 * 
	 * @param bytes
	 *            byte array
	 */
	...
	/**
	 * Creates a KeyValue from the specified byte array and offset. Presumes
	...
	/**
	 * Creates a KeyValue from the specified byte array, starting at offset, for
	 * length <code>length</code>, and a known <code>keyLength</code>.
	 * 
	 * @param bytes
	 *            byte array
	 * @param offset
	...
	public KeyValue(final byte[] row, final byte[] family,
			final byte[] qualifier, final long timestamp, Type type,
			final byte[] value) {
		this(row, family, qualifier, 0, qualifier == null ? 0
				: qualifier.length, timestamp, type, value, 0,
	 ...
	 * @throws IllegalArgumentException
	 */
	public KeyValue(byte[] row, byte[] family, byte[] qualifier, int qoffset,
			int qlength, long timestamp, Type type, byte[] value, int voffset,
			int vlength) {
		this(row, 0, row == null ? 0 : row.length, family, 0,
				family == null ? 0 : family.length, qualifier, qoffset,
				qlength, timestamp, type, value, voffset, vlength);
	}
	...
	public KeyValue(final byte[] row, final int roffset, final int rlength,
		...
		this.bytes = createByteArray(row, roffset, rlength, family, foffset,
				flength, qualifier, qoffset, qlength, timestamp, type, value,
				voffset, vlength);
	...
	/**
	 * Constructs an empty KeyValue structure, with specified sizes. This can be
	 * used to partially fill up KeyValues.
	 * <p>
	 * Column is split into two fields, family and qualifier.
	 * 
	 * @param rlength
	 ...
	 * @throws IllegalArgumentException
	 */
	public KeyValue(final int rlength, final int flength, final int qlength,
			final long timestamp, final Type type, final int vlength) {
		this.bytes = createEmptyByteArray(rlength, flength, qlength, timestamp,
				type, vlength);
		this.length = bytes.length;
		this.offset = 0;
	}
	...
	/**
	 * Create an empty byte[] representing a KeyValue All lengths are preset and
	...
	static byte[] createEmptyByteArray(final int rlength, int flength,
		...
		// Key length
		long longkeylength = KEY_INFRASTRUCTURE_SIZE + rlength + flength
				+ qlength;
		...
		byte[] bytes = new byte[KEYVALUE_INFRASTRUCTURE_SIZE + keylength
				+ vlength];
	...
	static byte[] createByteArray(final byte[] row, final int roffset,
			final int rlength, final byte[] family, final int foffset,
			int flength, final byte[] qualifier, final int qoffset,
			int qlength, final long timestamp, final Type type,
	 ...
	 */
	static byte[] createByteArray(final byte[] row, final int roffset,
			final int rlength, final byte[] column, final int coffset,
			int clength, final long timestamp, final Type type,
			final byte[] value, final int voffset, int vlength) {
		...
		if (column != null && column.length > 0) {
			...
			return createByteArray(row, roffset, rlength, null, 0, 0, null, 0,
					0, timestamp, type, value, voffset, vlength);
		}
		int flength = delimiteroffset - coffset;
		int qlength = clength - flength - 1;
		return createByteArray(row, roffset, rlength, column, coffset, flength,
				column, delimiteroffset + 1, qlength, timestamp, type, value,
				voffset, vlength);
	}
	...
	/**
	 * Creates a deep copy of this KeyValue, re-allocating the buffer. Same
	 * function as {@link #clone()}. Added for clarity vs shallowCopy()
	 * 
	...
	public KeyValue shallowCopy() {
		KeyValue shallowCopy = new KeyValue(this.bytes, this.offset,
				this.length);
	 ...
	 */
	/**
	 * Produces a string map for this key/value pair. Useful for programmatic
	 * use and manipulation of the data stored in an HLogKey, for example,
	 * printing as JSON. Values are left out due to their tendency to be large.
	 * If needed, they can be added manually.
	 * 
	...
	public static String keyToString(final byte[] b, final int o, final int l) {
		...
		String family = familylength == 0 ? "" : Bytes.toStringBinary(b,
				columnoffset, familylength);
		String qualifier = columnlength == 0 ? "" : Bytes.toStringBinary(b,
				columnoffset + familylength, columnlength - familylength);
	...
	/**
	 * Determines the total length of the KeyValue stored in the specified byte
	 * array and offset. Includes all headers.
	 * 
	 * @param bytes
	 *            byte array
	 * @param offset
	...
	private static int getLength(byte[] bytes, int offset) {
		return ROW_OFFSET + Bytes.toInt(bytes, offset)
	...
	public int getFamilyOffset(int rlength) {
		return this.offset + ROW_OFFSET + Bytes.SIZEOF_SHORT + rlength
				+ Bytes.SIZEOF_BYTE;
	}
	...
	public int getQualifierLength(int rlength, int flength) {
		return getKeyLength() - (KEY_INFRASTRUCTURE_SIZE + rlength + flength);
	}
	...
	public boolean isLatestTimestamp() {
		return Bytes.equals(getBuffer(), getTimestampOffset(),
				Bytes.SIZEOF_LONG, HConstants.LATEST_TIMESTAMP_BYTES, 0,
				Bytes.SIZEOF_LONG);
	}

	/**
	 * @return True if this is a "fake" KV created for internal seeking
	 *         purposes, which should not be seen by user code
	 */
	...
	/**
	 * Returns value in a new byte array. Primarily for use client-side. If
	 * server-side, use {@link #getBuffer()} with appropriate offsets and
	 * lengths instead to save on allocations.
	 * 
	...
	public boolean isDeleteColumnOrFamily() {
		int t = getType();
		return t == Type.DeleteColumn.getCode()
				|| t == Type.DeleteFamily.getCode();
	}
	 ...
	 * 
	 * If server-side, use {@link #getBuffer()} with appropriate offsets and
	 * lengths instead. Use {@link #getBuffer()} with appropriate offsets and
	 * lengths instead.
	 * 
	...
	public boolean matchingFamily(final byte[] family, int offset, int length) {
		...
		return Bytes.equals(family, offset, length, this.bytes,
				getFamilyOffset(), getFamilyLength());
	}
	...
	public boolean matchingQualifier(final byte[] qualifier, int offset,
			int length) {
		return Bytes.equals(qualifier, offset, length, this.bytes,
	...
	public boolean matchingRow(final byte[] row, int offset, int length) {
		return Bytes.equals(row, offset, length, this.bytes, getRowOffset(),
				getRowLength());
	}
	...
	public boolean matchingColumn(final byte[] family, final byte[] qualifier) {
		...
		return Bytes.equals(qualifier, 0, qualifier.length, this.bytes, o + fl,
				ql);
	}
	 ...
	 * @param llength
	 * @param lfamilylength
	 *            Offset of family delimiter in left column.
	 * @param right
	 * @param roffset
	 * @param rlength
	 * @param rfamilylength
	 *            Offset of family delimiter in right column.
	 * @return The result of the comparison.
	 */
	static int compareColumns(final byte[] left, final int loffset,
			final int llength, final int lfamilylength, final byte[] right,
			final int roffset, final int rlength, final int rfamilylength) {
		// Compare family portion first.
		int diff = Bytes.compareTo(left, loffset, lfamilylength, right,
				roffset, rfamilylength);
		if (diff != 0) {
			return diff;
		}
		// Compare qualifier portion
		return Bytes.compareTo(left, loffset + lfamilylength, llength
				- lfamilylength, right, roffset + rfamilylength, rlength
				- rfamilylength);
	}
	...
	public KeyValue createKeyOnly(boolean lenAsVal) {
		...
		if (lenAsVal) {
			Bytes.putInt(newBuffer, newBuffer.length - dataLen,
					this.getValueLength());
		}
	...
	private static int getRequiredDelimiter(final byte[] b, final int offset,
			final int length, final int delimiter) {
		int index = getDelimiter(b, offset, length, delimiter);
		if (index < 0) {
			throw new IllegalArgumentException("No " + (char) delimiter
					+ " in <" + Bytes.toString(b) + ">" + ", length=" + length
					+ ", offset=" + offset);
		}
		return index;
	}
	 ...
	 */
	static int getRequiredDelimiterInReverse(final byte[] b, final int offset,
			final int length, final int delimiter) {
		int index = getDelimiterInReverse(b, offset, length, delimiter);
		if (index < 0) {
			throw new IllegalArgumentException(
					".META. key must have two '"
							+ (char) delimiter
							+ "' "
	 ...
	 *         moving rightward.
	 */
	public static int getDelimiter(final byte[] b, int offset,
			final int length, final int delimiter) {
		if (b == null) {
	 ...
	 * considered the same as far as this Comparator is concerned. Hosts a
	 * {@link KeyComparator}.
	 */
	public static class KVComparator implements java.util.Comparator<KeyValue> {
		 ...
		 *         Used in hfile where indices are the Key portion of a
		 *         KeyValue.
		 */
		public KeyComparator getRawComparator() {
			return this.rawcomparator;
		...
		public int compareRows(final KeyValue left, final short lrowlength,
			...
			return getRawComparator().compareRows(left.getBuffer(),
					left.getRowOffset(), lrowlength, right.getBuffer(),
		...
		public int compareRows(final KeyValue left, final byte[] row) {
			return getRawComparator().compareRows(left.getBuffer(),
					left.getRowOffset(), left.getRowLength(), row, 0,
					row.length);
		}
		...
		public int compareColumns(final KeyValue left, final byte[] right,
			...
			return getRawComparator().compareColumns(left.getBuffer(), offset,
					length, left.getFamilyLength(offset), right, roffset,
					rlength, rfamilyoffset);
		}
		...
		int compareColumns(final KeyValue left, final short lrowlength,
			...
			return getRawComparator().compareColumns(left.getBuffer(),
					lfoffset, lclength, lfamilylength, right.getBuffer(),
					rfoffset, rclength, rfamilylength);
		}
		...
		public boolean matchingRowColumn(final KeyValue left,
			...
			// TsOffset = end of column data. just comparing Row+CF length of
			// each
			return ((left.getTimestampOffset() - left.getOffset()) == (right
					.getTimestampOffset() - right.getOffset()))
		...
		public boolean matchingRows(final KeyValue left, final byte[] right) {
			return Bytes.equals(left.getBuffer(), left.getRowOffset(),
					left.getRowLength(), right, 0, right.length);
		}
		 ...
		 * @return True if rows match.
		 */
		public boolean matchingRows(final KeyValue left,
				final short lrowlength, final KeyValue right,
				final short rrowlength) {
			return lrowlength == rrowlength
					&& Bytes.equals(left.getBuffer(), left.getRowOffset(),
							lrowlength, right.getBuffer(),
							right.getRowOffset(), rrowlength);
		}
		...
		public boolean matchingRows(final byte[] left, final int loffset,
				...
				final int rlength) {
			return Bytes
					.equals(left, loffset, llength, right, roffset, rlength);
		}

		/**
		 * Compares the row and timestamp of two keys Was called
		 * matchesWithoutColumn in HStoreKey.
		 * 
		 * @param right
		 *            Key to compare against.
		 * @return True if same row and timestamp is greater than the timestamp
		 *         in <code>right</code>
		 */
	...
	/**
	 * Creates a KeyValue that is last on the specified row id. That is, every
	 * other possible KeyValue for the given row would compareTo() less than the
	 * result of this call.
	 * 
	 * @param row
	 *            row key
	...
	/**
	 * Create a KeyValue that is smaller than all other possible KeyValues for
	 * the given row. That is any (valid) KeyValue on 'row' would sort _after_
	 * the result.
	 * 
	 * @param row
	 *            - row key (arbitrary byte array)
	...
	public static KeyValue createFirstOnRow(final byte[] row, int roffset,
			short rlength) {
		return new KeyValue(row, roffset, rlength, null, 0, 0, null, 0, 0,
				HConstants.LATEST_TIMESTAMP, Type.Maximum, null, 0, 0);
	}

	/**
	 * Creates a KeyValue that is smaller than all other KeyValues that are
	...
	/**
	 * Create a KeyValue for the specified row, family and qualifier that would
	 * be smaller than all other possible KeyValues that have the same
	 * row,family,qualifier. Used for seeking.
	 * 
	 * @param row
	 *            - row key (arbitrary byte array)
	 * @param family
	...
	public static KeyValue createFirstOnRow(final byte[] row,
		...
		return new KeyValue(row, family, qualifier,
				HConstants.LATEST_TIMESTAMP, Type.Maximum);
	}

	/**
	 * Create a Delete Family KeyValue for the specified row and family that
	 * would be smaller than all other possible Delete Family KeyValues that
	 * have the same row and family. Used for seeking.
	 * 
	 * @param row
	 *            - row key (arbitrary byte array)
	 * @param family
	 ...
	 * @param ts
	 *            - timestamp
	 * @return First possible key on passed <code>row</code>, column and
	 *         timestamp
	 */
	...
	/**
	 * Create a KeyValue for the specified row, family and qualifier that would
	 * be smaller than all other possible KeyValues that have the same row,
	 * family, qualifier. Used for seeking.
	 * 
	 * @param row
	 *            row key
	 * @param roffset
	...
	/**
	 * Create a KeyValue for the specified row, family and qualifier that would
	 * be larger than or equal to all other possible KeyValues that have the
	 * same row, family, qualifier. Used for reseeking.
	 * 
	 * @param row
	 *            row key
	 * @param roffset
	 ...
	 */
	public static KeyValue createLastOnRow(final byte[] row, final int roffset,
			final int rlength, final byte[] family, final int foffset,
			final int flength, final byte[] qualifier, final int qoffset,
			final int qlength) {
		return new KeyValue(row, roffset, rlength, family, foffset, flength,
				qualifier, qoffset, qlength, HConstants.OLDEST_TIMESTAMP,
	...
	public KeyValue createLastOnRowCol() {
		return new KeyValue(bytes, getRowOffset(), getRowLength(), bytes,
				getFamilyOffset(), getFamilyLength(), bytes,
				getQualifierOffset(), getQualifierLength(),
	...
	public KeyValue createFirstOnRowColTS(long ts) {
		return new KeyValue(bytes, getRowOffset(), getRowLength(), bytes,
				getFamilyOffset(), getFamilyLength(), bytes,
				getQualifierOffset(), getQualifierLength(), ts, Type.Maximum,
				bytes, getValueOffset(), getValueLength());
	}
	...
	public static class RootKeyComparator extends MetaKeyComparator {
		public int compareRows(byte[] left, int loffset, int llength,
			...
			int leftFarDelimiter = getDelimiterInReverse(left,
					lmetaOffsetPlusDelimiter, llength - metalength,
			...
			int result = super.compareRows(left, lmetaOffsetPlusDelimiter,
					leftFarDelimiter - lmetaOffsetPlusDelimiter, right,
					rmetaOffsetPlusDelimiter, rightFarDelimiter
							- rmetaOffsetPlusDelimiter);
			if (result != 0) {
				return result;
			}
			...
			leftFarDelimiter++;
			rightFarDelimiter++;
			result = compareRowid(left, leftFarDelimiter, llength
					- (leftFarDelimiter - loffset), right, rightFarDelimiter,
	...
	public static class MetaKeyComparator extends KeyComparator {
		public int compareRows(byte[] left, int loffset, int llength,
			...
			int result = Bytes.compareTo(left, loffset,
					leftDelimiter - loffset, right, roffset, rightDelimiter
							- roffset);
			if (result != 0) {
				return result;
			}
			...
			leftDelimiter++;
			rightDelimiter++;
			int leftFarDelimiter = getRequiredDelimiterInReverse(left,
					leftDelimiter, llength - (leftDelimiter - loffset),
					HRegionInfo.DELIMITER);
			int rightFarDelimiter = getRequiredDelimiterInReverse(right,
					rightDelimiter, rlength - (rightDelimiter - roffset),
					HRegionInfo.DELIMITER);
			// Now compare middlesection of row.
			result = super.compareRows(left, leftDelimiter, leftFarDelimiter
		...
		protected int compareRowid(byte[] left, int loffset, int llength,
				byte[] right, int roffset, int rlength) {
			return Bytes.compareTo(left, loffset, llength, right, roffset,
					rlength);
		}
	}
	...
	public static interface SamePrefixComparator<T> {
		 ...
		 */
		public int compareIgnoringPrefix(int commonPrefix, T left, int loffset,
				int llength, T right, int roffset, int rlength);
	}
	...
	public static class KeyComparator implements RawComparator<byte[]>,
		...
		public int compare(byte[] left, int loffset, int llength, byte[] right,
			...
			// Compare the rest of the two KVs without making any assumptions
			// about
			// the common prefix. This function will not compare rows anyway, so
			// we
		...
		public int compareIgnoringPrefix(int commonPrefix, byte[] left,
			...
			if (commonPrefix < ROW_LENGTH_SIZE) {
				...
				comparisonResult = compareRows(left, loffset + ROW_LENGTH_SIZE,
						lrowlength, right, roffset + ROW_LENGTH_SIZE,
						rrowlength);
			} else { // the row length is the same
				rrowlength = lrowlength;
				if (commonPrefix < ROW_LENGTH_SIZE + rrowlength) {
					// The rows are not the same. Exclude the common prefix and
					// compare
					// the rest of the two rows.
					int common = commonPrefix - ROW_LENGTH_SIZE;
					comparisonResult = compareRows(left, loffset + common
							+ ROW_LENGTH_SIZE, lrowlength - common, right,
							roffset + common + ROW_LENGTH_SIZE, rrowlength
									- common);
				}
			}
			...
			return compareWithoutRow(commonPrefix, left, loffset, llength,
					right, roffset, rlength, lrowlength);
		}

		/**
		 * Compare columnFamily, qualifier, timestamp, and key type (everything
		 * except the row). This method is used both in the normal comparator
		 * and the "same-prefix" comparator. Note that we are assuming that row
		 * portions of both KVs have already been parsed and found identical,
		 * and we don't validate that assumption here.
		 * 
		 * @param commonPrefix
		 *            the length of the common prefix of the two key-values
		 *            being compared, including row length and row
		 */
		private int compareWithoutRow(int commonPrefix, byte[] left,
				int loffset, int llength, byte[] right, int roffset,
				int rlength, short rowlength) {
			/***
			 * KeyValue Format and commonLength:
			 * |_keyLen_|_valLen_|_rowLen_|_rowKey_
			 * |_famiLen_|_fami_|_Quali_|....
			 * ------------------|-------commonLength--------|--------------
			 */
			...
			// the
			// latest in the sorted order, regardless of the timestamp. This is
			// used
			// for specifying the last key/value in a given row, because there
			// is no
			// "lexicographically last column" (it would be infinitely long).
			// The
			...
			// If left family size is not equal to right family size, we need
			// not
			...
			if (commonPrefix > 0) {
				...
				if (!sameFamilySize) {
					...
					common = Math.min(common,
							Math.min(lfamilylength, rfamilylength));
				} else {
					common = Math.min(common,
							Math.min(lcolumnlength, rcolumnlength));
				}
			}
			if (!sameFamilySize) {
				// comparing column family is enough.
				return Bytes.compareTo(left, lfamilyoffset + common,
						lfamilylength - common, right, rfamilyoffset + common,
						rfamilylength - common);
			}
			// Compare family & qualifier together.
			final int comparison = Bytes.compareTo(left,
					lfamilyoffset + common, lcolumnlength - common, right,
					rfamilyoffset + common, rcolumnlength - common);
			if (comparison != 0) {
				return comparison;
			}
			return compareTimestampAndType(left, loffset, llength, right,
					roffset, rlength, ltype, rtype);
		}

		private int compareTimestampAndType(byte[] left, int loffset,
				int llength, byte[] right, int roffset, int rlength,
				byte ltype, byte rtype) {
			int compare;
			if (!this.ignoreTimestamp) {
				// Get timestamps.
				long ltimestamp = Bytes.toLong(left, loffset
						+ (llength - TIMESTAMP_TYPE_SIZE));
				long rtimestamp = Bytes.toLong(right, roffset
						+ (rlength - TIMESTAMP_TYPE_SIZE));
			...
			if (!this.ignoreType) {
				// Compare types. Let the delete types sort ahead of puts; i.e.
				// types
		...
		protected int compareColumns(byte[] left, int loffset, int llength,
				final int lfamilylength, byte[] right, int roffset,
				int rlength, final int rfamilylength) {
			return KeyValue.compareColumns(left, loffset, llength,
					lfamilylength, right, roffset, rlength, rfamilylength);
		}

		int compareTimestamps(final long ltimestamp, final long rtimestamp) {
			// The below older timestamps sorting ahead of newer timestamps
			// looks
