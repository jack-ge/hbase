 ...
 * thereafter into the HBase cluster. Knows how to locate regions out on the
 * cluster, keeps a cache of locations and then knows how to recalibrate after
 * they move. {@link HConnectionManager} manages instances of this class.
 * 
 * <p>
 * HConnections are used by {@link HTable} mostly but also by {@link HBaseAdmin}, {@link CatalogTracker}, and {@link ZooKeeperWatcher}. HConnection instances
 * can be shared. Sharing is usually what you want because rather than each
 * HConnection instance having to do its own discovery of regions out on the
 * cluster, instead, all clients get to share the one cache of locations.
 * Sharing makes cleanup of HConnections awkward. See {@link HConnectionManager}
...
public interface HConnection extends Abortable, Closeable {
	 ...
	 *             interface (ZooKeeper is an implementation detail). Deprecated
	 *             in HBase 0.94
	 */
	@Deprecated
	 ...
	 *             interface (master is an implementation detail). Master
	 *             functions are available from HConnection or HBaseAdmin,
	 *             without having to use directly the master. Deprecated in
	 *             HBase 0.94
	 */
	@Deprecated
	 ...
	 * possible. This happens when a table has a lot of regions that must be
	 * processed.
	 * 
	 * @param tableName
	 *            table name
	 ...
	 * only exists as part of the META table's region info.
	 * 
	...
	/**
	 * Find the location of the region of <i>tableName</i> that <i>row</i> lives
	 * in.
	 * 
	 * @param tableName
	...
	/**
	 * Find the location of the region of <i>tableName</i> that <i>row</i> lives
	 * in, ignoring any value that might be in the cache.
	 * 
	 * @param tableName
	 *            name of the table <i>row</i> is in
	 * @param row
	 ...
	 */
	public HRegionInterface getHRegionConnection(final String hostname,
			final int port, boolean getMaster) throws IOException;
	 ...
	 * this method will manage the process of doing retries with timed waits and
	 * refinds of missing regions.
	 * 
	 * @param <T>
	 ...
	 *            you can test here for partial results, and to determine which
	 *            actions processed successfully.
	 * @throws IOException
	 *             if there are problems talking to META. Per-item exceptions
	...
	/**
	 * Executes the given
	 * {@link org.apache.hadoop.hbase.client.coprocessor.Batch.Call} callable
	 * for each row in the given list and invokes
	 ...
	 * @param rows
	 *            a list of row keys for which the callable should be invoked
	 * @param tableName
	...
	public <T extends CoprocessorProtocol, R> void processExecs(
			final Class<T> protocol, List<byte[]> rows, final byte[] tableName,
			ExecutorService pool, final Batch.Call<T, R> call,
	 ...
	 * for the given table's all HTable instances within this connection. By
	 * default, the cache prefetch is enabled.
	 * 
	 * @param tableName
	 *            name of table to configure.
	 * @param enable
	 *            Set to true to enable region cache prefetch.
	 */
	 ...
	 * @return true if table's region cache prefetch is enabled. Otherwise it is
	 *         disabled.
	 */
	 ...
	 * 
	 * @param tableName
	 *            name of the table to perform region cache prewarm.
	 * @param regions
	 *            a region map.
	 */
	...
	public abstract void clearClusterCache();

	public abstract void clearClusterCache(String paramString);
}
