...
public class AggregationClient {
	 ...
	 */
	public <R, S> R max(final byte[] tableName,
		...
		try {
			...
			table.coprocessorExec(AggregateProtocol.class, scan.getStartRow(),
					scan.getStopRow(), new Batch.Call<AggregateProtocol, R>() {
						@Override
						public R call(AggregateProtocol instance)
								throws IOException {
	 ...
	 * @throws Throwable
	 */
	public <R, S> R min(final byte[] tableName,
			final ColumnInterpreter<R, S> ci, final Scan scan) throws Throwable {
		validateParameters(scan);
	 ...
	 * @throws Throwable
	 */
	public <R, S> S sum(final byte[] tableName,
			final ColumnInterpreter<R, S> ci, final Scan scan) throws Throwable {
		validateParameters(scan);
		...
		try {
			...
			table.coprocessorExec(AggregateProtocol.class, scan.getStartRow(),
					scan.getStopRow(), new Batch.Call<AggregateProtocol, S>() {
						@Override
						public S call(AggregateProtocol instance)
								throws IOException {
			...
			table.coprocessorExec(AggregateProtocol.class, scan.getStartRow(),
					...
					new Batch.Call<AggregateProtocol, Pair<List<S>, Long>>() {
						...
						public Pair<List<S>, Long> call(
								AggregateProtocol instance) throws IOException {
	 ...
	 * @throws Throwable
	 */
	public <R, S> double std(final byte[] tableName,
	...
	public <R, S> R median(final byte[] tableName, ColumnInterpreter<R, S> ci,
		...
		Pair<NavigableMap<byte[], List<S>>, List<S>> p = getMedianArgs(
				tableName, ci, scan);
		byte[] startRow = null;
