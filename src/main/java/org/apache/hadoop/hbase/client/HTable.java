 ...
 * 
 * <p>
 * In case of writes (Put, Delete), the underlying write buffer can be corrupted
 * if multiple threads contend over a single HTable instance.
 * 
 * <p>
 * In case of reads, some fields used by a Scan are shared among all threads.
 * The HTable implementation can either not contract to be safe in case of a Get
 * 
 * <p>
 * To access a table in a multi threaded environment, please consider using the
 * {@link HTablePool} class to create your HTable instances.
 * 
 * <p>
 * Instances of HTable passed the same {@link Configuration} instance will share
 * connections to servers out on the cluster and to the zookeeper ensemble as
 * well as caches of region locations. This is usually a *good* thing and it is
 * recommended to reuse the same configuration object for all your tables. This
 * happens because they will all share the same underlying {@link HConnection}
 * instance. See {@link HConnectionManager} for more on how this mechanism
 * works.
 * 
 * <p>
 * {@link HConnection} will read most of the configuration it needs from the
 * passed {@link Configuration} on initial construction. Thereafter, for
 * settings such as <code>hbase.client.pause</code>,
 * <code>hbase.client.retries.number</code>, and
 * <code>hbase.client.rpc.maxattempts</code> updating their values in the passed
 * {@link Configuration} subsequent to {@link HConnection} construction will go
 * unnoticed. To run with changed values, make a new {@link HTable} passing a
 * new {@link Configuration} instance that has the new configuration.
 * 
 * <p>
 * Note that this class implements the {@link Closeable} interface. When a
 * HTable instance is no longer required, it *should* be closed in order to
 * ensure that the underlying resources are promptly released. Please note that
 * the close method can throw java.io.IOException that must be handled.
 * 
...
public class HTable implements HTableInterface {
	...
	/**
	 * Creates an object to access a HBase table. Internally it creates a new
	 * instance of {@link Configuration} and a new client to zookeeper as well
	 * as other resources. It also comes up with a fresh view of the cluster and
	 * must do discovery from scratch of region locations; i.e. it will not make
	 * use of already-cached region locations if available. Use only when being
	 * quick and dirty.
	 * 
	 * @throws IOException
	...
	/**
	 * Creates an object to access a HBase table. Shares zookeeper connection
	 * and other resources with other HTable instances created with the same
	 * <code>conf</code> instance. Uses already-populated region cache if one is
	 * available, populated by any other HTable instances sharing this
	 * <code>conf</code> instance. Recommended.
	 * 
	 * @param conf
	...
	public HTable(Configuration conf, final byte[] tableName)
		...
		this.pool = new ThreadPoolExecutor(1, maxThreads, keepAliveTime,
	 ...
	 * <code>conf</code> instance. Uses already-populated region cache if one is
	 * available, populated by any other HTable instances sharing this
	 * <code>conf</code> instance. Use this constructor when the ExecutorService
	 ...
	 * Creates an object to access a HBase table. Shares zookeeper connection
	 * and other resources with other HTable instances created with the same
	 * <code>connection</code> instance. Use this constructor when the
	 * ExecutorService and HConnection instance are externally managed.
	 * 
	 * @param tableName
	 *            Name of the table.
	 * @param connection
	...
	/**
	 * Tells whether or not a table is enabled or not. This method creates a new
	 * HBase configuration, so it might make your unit tests fail due to
	...
	public HRegionLocation getRegionLocation(final String row)
			throws IOException {
		return connection.getRegionLocation(tableName, Bytes.toBytes(row),
				false);
	}
	 ...
	 *            Row to find.
	 * @param reload
	 *            whether or not to reload information or just use cached
	 *            information
	 * @return Location of the row.
	 * @throws IOException
	 *             if a remote or network exception occurs
	 ...
	 * 
	 * @deprecated Use {@link Scan#setCaching(int)} and
	 *             {@link Scan#getCaching()}
	 */
	 ...
	 * @throws IOException
	 *             if a remote or network exception occurs
	 * @deprecated Use {@link #getRegionLocations()} or
	 *             {@link #getStartEndKeys()}
	 */
	public Map<HRegionInfo, HServerAddress> getRegionsInfo() throws IOException {
		...
		for (Map.Entry<HRegionInfo, ServerName> entry : regionLocations
				.entrySet()) {
			HServerAddress server = new HServerAddress();
			ServerName serverName = entry.getValue();
			if (serverName != null && serverName.getHostAndPort() != null) {
				server = new HServerAddress(
						Addressing
								.createInetSocketAddressFromHostAndPortStr(serverName
										.getHostAndPort()));
			}
	...
	public List<HRegionLocation> getRegionsInRange(final byte[] startKey,
		...
		do {
			HRegionLocation regionLocation = getRegionLocation(currentKey,
					false);
	 ...
	 * {@link #deserializeRegionInfo deserializeRegionInfo} to deserialize
	 * regions information from a {@link DataInput}, then call this method to
	 * load them to cache.
	 * 
	 * <pre>
	 * {
	 ...
	 * 
	 * @param regionMap
	 *            This piece of regions information will be loaded to region
	 *            cache.
	 */
	...
	/**
	 * Serialize the regions information of this table and output to
	 * <code>out</code>.
	 * <p>
	 ...
	 * region info to a file. MR job can ship a copy of the meta for the table
	 ...
	 * 
	 * @param out
	 *            {@link DataOutput} to serialize this object into.
	 * @throws IOException
	 *             if a remote or network exception occurs
	 */
	 ...
	 * 
	 * <p>
	 * It behaves similarly as {@link #getRegionsInfo getRegionsInfo}, except
	 * that it loads the region map from a {@link DataInput} object.
	 * 
	 * <p>
	 * It is supposed to be followed immediately by {@link #prewarmRegionCache
	 * prewarmRegionCache}.
	 * 
	 * <p>
	...
	public Result getRowOrBefore(final byte[] row, final byte[] family)
			throws IOException {
		return new ServerCallable<Result>(connection, tableName, row,
				operationTimeout) {
			public Result call() throws IOException {
				return server.getClosestRowBefore(location.getRegionInfo()
						.getRegionName(), row, family);
			}
		}.withRetries();
	}
	...
	public Result get(final Get get) throws IOException {
		return new ServerCallable<Result>(connection, tableName, get.getRow(),
			...
			public Result call() throws IOException {
				return server
						.get(location.getRegionInfo().getRegionName(), get);
			}
		}.withRetries();
	}
	...
	public Result append(final Append append) throws IOException {
		...
		return new ServerCallable<Result>(connection, tableName,
				append.getRow(), operationTimeout) {
			public Result call() throws IOException {
				return server.append(location.getRegionInfo().getRegionName(),
						append);
			}
		}.withRetries();
	}
	...
	public Result increment(final Increment increment) throws IOException {
		...
		return new ServerCallable<Result>(connection, tableName,
				increment.getRow(), operationTimeout) {
			public Result call() throws IOException {
				return server.increment(location.getRegionInfo()
						.getRegionName(), increment);
			}
		}.withRetries();
	}
	...
	public long incrementColumnValue(final byte[] row, final byte[] family,
		...
		return new ServerCallable<Long>(connection, tableName, row,
			...
			public Long call() throws IOException {
				return server.incrementColumnValue(location.getRegionInfo()
						.getRegionName(), row, family, qualifier, amount,
						writeToWAL);
			}
		}.withRetries();
	}
	...
	public boolean checkAndPut(final byte[] row, final byte[] family,
		...
		return new ServerCallable<Boolean>(connection, tableName, row,
				operationTimeout) {
			public Boolean call() throws IOException {
				return server.checkAndPut(location.getRegionInfo()
						.getRegionName(), row, family, qualifier, value, put) ? Boolean.TRUE
						: Boolean.FALSE;
			}
		}.withRetries();
	}
	...
	public boolean checkAndDelete(final byte[] row, final byte[] family,
		...
		return new ServerCallable<Boolean>(connection, tableName, row,
			...
			public Boolean call() throws IOException {
				return server
						.checkAndDelete(location.getRegionInfo()
								.getRegionName(), row, family, qualifier,
								value, delete) ? Boolean.TRUE : Boolean.FALSE;
			}
		}.withRetries();
	}
	...
	public boolean exists(final Get get) throws IOException {
		return new ServerCallable<Boolean>(connection, tableName, get.getRow(),
				operationTimeout) {
			public Boolean call() throws IOException {
				return server.exists(location.getRegionInfo().getRegionName(),
						get);
			}
		}.withRetries();
	}
	...
	public void flushCommits() throws IOException {
		try {
			...
			try {
				this.connection.processBatch(writeBuffer, tableName, pool,
						results);
			} catch (InterruptedException e) {
				throw new IOException(e);
			} finally {
				...
				// contains
				// only failed records results are returned in the same order as
				// the
				// requests in list walk the list backwards, so we can remove
				// from list
	...
	public RowLock lockRow(final byte[] row) throws IOException {
		return new ServerCallable<RowLock>(connection, tableName, row,
				operationTimeout) {
	 ...
	 * the application dies before pending writes get flushed to HBase, data
	 * will be lost.
	 * <p>
	 ...
	 * clearBufferOnFail to true to erase the buffer after
	...
	/**
	 * Enable or disable region cache prefetch for the table. It will be applied
	 * for the given table's all HTable instances who share the same connection.
	 * By default, the cache prefetch is enabled.
	 * 
	 * @param tableName
	 *            name of table to configure.
	 * @param enable
	 *            Set to true to enable region cache prefetch. Or set to false
	 *            to disable it.
	 * @throws IOException
	 */
	 ...
	 * @param tableName
	 *            name of table to check
	 * @return true if table's region cache prefecth is enabled. Otherwise it is
	 *         disabled.
	 * @throws IOException
	 */
	...
	public <T extends CoprocessorProtocol> T coprocessorProxy(
		...
		return (T) Proxy.newProxyInstance(this.getClass().getClassLoader(),
				new Class[] { protocol }, new ExecRPCInvoker(configuration,
						connection, protocol, tableName, row));
	}
