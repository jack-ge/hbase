...
/**
 * Scanner scans both the memstore and the HStore. Coalesce KeyValue stream into
 * List<KeyValue> for a single row.
 */
public class StoreScanner extends NonLazyKeyValueScanner implements
	...
	public synchronized boolean next(List<KeyValue> outResult, int limit)
			throws IOException {
		boolean result = nextInternal(outResult, limit);

		if ((null != this.store)
				&& (this.store.getFamily().isLobStoreEnabled())
				&& (this.scan.isResolveReference())) {
			updateFromBlobStore(outResult);
		}

		return result;
	}

	private void updateFromBlobStore(List<KeyValue> result) throws IOException {
		if ((result == null) || (result.size() == 0)) {
			return;
		}
		int last = result.size() - 1;
		for (int i = 0; i < result.size(); i++) {
			KeyValue keyValue = (KeyValue) result.get(i);
			if (null == keyValue) {
				break;
			}
			if (keyValue.getType() == KeyValue.Type.Reference.getCode()) {
				BlobStore blobStore = BlobStoreManager.getInstance()
						.getBlobStore(this.store.getTableName(),
								this.store.getColumnFamilyName());

				KeyValue realKeyValue = null != blobStore ? blobStore
						.resolve(keyValue) : null;

				if (null == realKeyValue) {
					result.set(i, result.get(last));
					result.set(last--, null);
				} else {
					result.set(i, realKeyValue);
				}
			}
		}
		for (int i = result.size() - 1; i > last; i--)
			result.remove(i);
	}
	 ...
	 */
	public synchronized boolean nextInternal(List<KeyValue> outResult, int limit)
			throws IOException {
	...
	public synchronized boolean next(List<KeyValue> outResult, int limit,
		...
		KeyValue.KVComparator comparator = store != null ? store
				.getComparator() : null;
		...
		try {
			LOOP: while ((kv = this.heap.peek()) != null) {
				...
				assert prevKV == null || comparator == null
						|| comparator.compare(prevKV, kv) <= 0 : "Key "
						+ prevKV + " followed by a " + "smaller key " + kv
						+ " in cf " + store;
				prevKV = kv;
			...
			if (cumulativeMetric > 0 && metric != null) {
				RegionMetricsStorage.incrNumericMetric(this.metricNamePrefix
	...
	private boolean checkReseek() throws IOException {
		if (this.heap == null && this.lastTop != null) {
			...
			if (this.heap.peek() == null
					|| store.comparator.compareRows(this.lastTop,
							this.heap.peek()) != 0) {
