 ...
 * locking is handled at the HRegion level. Store just provides services to
 * manage sets of StoreFiles. One of the most important of those services is
 * compaction services where files are aggregated once they pass a configurable
 * threshold.
 * 
 * <p>
...
public class Store extends SchemaConfigured implements HeapSize {
	...
	/*
	 * The default priority for user-specified compaction requests. The user
	 * gets top priority unless we have blocking compactions. (Pri <= 0)
	 */
	 ...
	 * atomically replaced when its contents change.
	 */
	...
	private final CopyOnWriteArraySet<ChangedReadersObserver> changedReaderObservers = new CopyOnWriteArraySet<ChangedReadersObserver>();
	 ...
	 * @param region
	 * @param family
	 *            HColumnDescriptor for this column
	 * @param fs
	 *            file system object
	 * @param confParam
	 *            configuration object failed. Can be null.
	 * @throws IOException
	 */
	protected Store(Path basedir, HRegion region, HColumnDescriptor family,
			FileSystem fs, Configuration conf) throws IOException {
		super(conf, region.getRegionInfo().getTableNameAsString(), Bytes
				.toString(family.getName()));
		HRegionInfo info = region.getRegionInfo();
		this.fs = fs;
		Path p = getStoreHomedir(basedir, info.getEncodedName(),
				family.getName());
		...
		this.dataBlockEncoder = new HFileDataBlockEncoderImpl(
				family.getDataBlockEncodingOnDisk(),
		...
		long timeToPurgeDeletes = Math.max(
				conf.getLong("hbase.hstore.time.to.purge.deletes", 0), 0);
		LOG.info("time to purge deletes set to " + timeToPurgeDeletes
				+ "ms in store " + this);
		scanInfo = new ScanInfo(family, ttl, timeToPurgeDeletes,
				this.comparator);
		...
		this.maxCompactSize = conf.getLong("hbase.hstore.compaction.max.size",
				Long.MAX_VALUE);
		...
		if (Store.closeCheckInterval == 0) {
			Store.closeCheckInterval = conf
					.getInt("hbase.hstore.close.check.interval",
							10 * 1000 * 1000 /* 10 MB */);
		}
		String majorCompactionCron = this.conf
				.get(HConstants.HBASE_REGION_MAJOR_COMPACTION_CRON);
		if ((majorCompactionCron != null) && (!majorCompactionCron.isEmpty())) {
			try {
				this.majorcompactionCronTrigger = new CronTrigger(toString(),
						"MajorCompaction", majorCompactionCron);

				this.majorcompactionCronTrigger.computeFirstFireTime(null);
			} catch (Exception e) {
				LOG.warn(
						new StringBuilder()
								.append("Failed to parse cron string: ")
								.append(majorCompactionCron)
								.append(". Will use major compaction interval instead. Error")
								.toString(), e);
			}
		}
	 ...
	 * @throws IOException
	 */
	Path createStoreHomeDir(final FileSystem fs, final Path homedir)
			throws IOException {
		if (!fs.exists(homedir)
				&& !HBaseFileSystem.makeDirOnFileSystem(fs, homedir)) {
	...
	/**
	 * @param parentRegionDirectory
	 *            directory for the parent region
	 * @param family
	...
	/**
	 * Creates an unsorted list of StoreFile loaded in parallel from the given
	 * directory.
	 * 
	 * @throws IOException
	 */
	private List<StoreFile> loadStoreFiles() throws IOException {
		...
		ThreadPoolExecutor storeFileOpenerThreadPool = this.region
				.getStoreFileOpenAndCloseThreadPool("StoreFileOpenerThread-"
		...
		for (int i = 0; i < files.length; i++) {
			...
			if (!HFileLink.isHFileLink(p)
					&& this.fs.getFileStatus(p).getLen() <= 0) {
				LOG.warn("Skipping " + p
	...
	/**
	 * Removes a kv from the memstore. The KeyValue is removed only if its key &
	 * memstoreTS matches the key & memstoreTS value of the kv parameter.
	 * 
	 * @param kv
	 */
	...
	void assertBulkLoadHFileOk(Path srcPath) throws IOException {
		...
		try {
			...
			reader = HFile.createReader(srcPath.getFileSystem(conf), srcPath,
					cacheConf);
			reader.loadFileInfo();

			byte[] firstKey = reader.getFirstRowKey();
			byte[] lk = reader.getLastKey();
			byte[] lastKey = (lk == null) ? null : KeyValue
					.createKeyValueFromKey(lk).getRow();

			LOG.debug("HFile bounds: first=" + Bytes.toStringBinary(firstKey)
					+ " last=" + Bytes.toStringBinary(lastKey));
			LOG.debug("Region bounds: first="
					+ Bytes.toStringBinary(region.getStartKey()) + " last="
					+ Bytes.toStringBinary(region.getEndKey()));

			HRegionInfo hri = region.getRegionInfo();
			if (!hri.containsRange(firstKey, lastKey)) {
				throw new WrongRegionException("Bulk load file "
			...
			if (verifyBulkLoads) {
				...
				do {
					...
					if (prevKV != null) {
						if (Bytes.compareTo(prevKV.getBuffer(),
								prevKV.getRowOffset(), prevKV.getRowLength(),
								kv.getBuffer(), kv.getRowOffset(),
								kv.getRowLength()) > 0) {
							throw new InvalidHFileException(
									"Previous row is greater than"
											+ " current row: path="
											+ srcPath
											+ " previous="
											+ Bytes.toStringBinary(prevKV
													.getKey()) + " current="
											+ Bytes.toStringBinary(kv.getKey()));
						}
						if (Bytes.compareTo(prevKV.getBuffer(),
								prevKV.getFamilyOffset(),
								prevKV.getFamilyLength(), kv.getBuffer(),
								kv.getFamilyOffset(), kv.getFamilyLength()) != 0) {
							throw new InvalidHFileException(
									"Previous key had different"
											+ " family compared to current key: path="
											+ srcPath
											+ " previous="
											+ Bytes.toStringBinary(prevKV
													.getFamily())
	...
	void bulkLoadHFile(String srcPathStr) throws IOException {
		...
		FileSystem desFs = fs instanceof HFileSystem ? ((HFileSystem) fs)
				.getBackingFs() : fs;
		...
		LOG.info("Successfully loaded store file " + srcPath + " into store "
				+ this + " (new location: " + dstPath + ")");
	}

	/**
	 * Get a temporary path in this region. These temporary files will get
	 * cleaned up when the region is re-opened if they are still around.
	 */
	 ...
	 * 
	 * We don't need to worry about subsequent requests because the HRegion
	...
	ImmutableList<StoreFile> close() throws IOException {
		...
		try {
			...
			if (!result.isEmpty()) {
				// initialize the thread pool for closing store files in
				// parallel.
	...
	/**
	 * Snapshot this stores memstore. Call before running {@link
	 * #flushCache(long, SortedSet<KeyValue>)} so it has some work to do.
	 */
	void snapshot() {
	...
	protected Path flushCache(final long logCacheFlushId,
		...
		if (this.family.isLobStoreEnabled()) {
	...
	private Path internalFlushCache(final SortedSet<KeyValue> set,
		...
		KeyValueScanner memstoreScanner = new CollectionBackedScanner(set,
				this.comparator);
		if (getHRegion().getCoprocessorHost() != null) {
			scanner = getHRegion().getCoprocessorHost().preFlushScannerOpen(
					this, memstoreScanner);
		}
		if (scanner == null) {
			Scan scan = new Scan();
		...
		if (getHRegion().getCoprocessorHost() != null) {
			InternalScanner cpScanner = getHRegion().getCoprocessorHost()
					.preFlush(this, scanner);
		...
		try {
			...
			// TODO: We can fail in the below block before we complete adding
			// this
			...
			synchronized (flushLock) {
				...
				try {
					...
					do {
						...
						if (!kvs.isEmpty()) {
							for (KeyValue kv : kvs) {
								// If we know that this KV is going to be
								// included always, then let us
								// set its memstoreTS to 0. This will help us
								// save space when writing to disk.
								if (kv.getMemstoreTS() <= smallestReadPoint) {
									// let us not change the original KV. It
									// could be in the memstore
									// changing its memstoreTS could affect
									// other threads/scanners.
									kv = kv.shallowCopy();
									kv.setMemstoreTS(0);
								}
								writer.append(kv);
					...
					// Write out the log sequence number that corresponds to
					// this output
					// hfile. The hfile is current up to and including
					// logCacheFlushId.
					status.setStatus("Flushing " + this
							+ ": appending metadata");
					writer.appendMetadata(logCacheFlushId, false);
					status.setStatus("Flushing " + this
							+ ": closing flushed file");
					writer.close();
				}
			}
		} finally {
		...
		if (LOG.isInfoEnabled()) {
			LOG.info("Flushed " + ", sequenceid=" + logCacheFlushId
	...
	private StoreFile commitFile(final Path path, final long logCacheFlushId,
		...
		StoreFile sf = new StoreFile(this.fs, dstPath, this.conf,
		...
		// This increments the metrics associated with total flushed bytes for
		// this
		...
		// HRegion.internalFlushcache, which indirectly calls this to actually
		// do
		// the flushing through the StoreFlusherImpl class
		getSchemaMetrics().updatePersistentStoreMetric(
				SchemaMetrics.StoreMetricType.FLUSH_SIZE,
				flushedSize.longValue());
		if (LOG.isInfoEnabled()) {
			LOG.info("Added " + sf + ", entries=" + r.getEntries()
					+ ", sequenceid=" + logCacheFlushId + ", filesize="
	...
	public StoreFile.Writer createWriterInTmp(int maxKeyCount,
		...
		StoreFile.Writer w = new StoreFile.WriterBuilder(conf, writerCacheConf,
				fs, blocksize).withOutputDir(region.getTmpDir())
	...
	/*
	 * Change storefiles adding into place the Reader produced by this new
	 * flush.
	 * 
	 * @param sf
	 * 
	...
	private boolean updateStorefiles(final StoreFile sf,
			final SortedSet<KeyValue> set) throws IOException {
		this.lock.writeLock().lock();
		try {
	...
	protected List<KeyValueScanner> getScanners(boolean cacheBlocks,
		...
		List<StoreFileScanner> sfScanners = StoreFileScanner
				.getScannersForStoreFiles(storeFiles, cacheBlocks, isGet,
	 ...
	 * 
	 * Existing StoreFiles are not destroyed until the new compacted StoreFile
	 * is completely written-out to disk.
	 * 
	 * <p>
	 * The compactLock prevents multiple simultaneous compactions. The
	 * structureLock prevents us from interfering with other write operations.
	 * 
	 * <p>
	 * We don't want to hold the structureLock for the whole time, as a
	 * compact() can be lengthy and we want to allow cache-flushes during this
	 * period.
	 * 
	 * @param cr
	...
	StoreFile compact(CompactionRequest cr) throws IOException {
		...
		synchronized (filesCompacting) {
			...
			Preconditions.checkArgument(filesCompacting
					.containsAll(filesToCompact));
		}
		...
		LOG.info("Starting compaction of " + filesToCompact.size()
				+ " file(s) in " + this + " of "
				+ this.region.getRegionInfo().getRegionNameAsString()
				+ " into tmpdir=" + region.getTmpDir() + ", seqid=" + maxId
				+ ", totalSize=" + StringUtils.humanReadableInt(cr.getSize()));
		...
		LOG.info("Completed"
				...
				+ ", size="
				+ (sf == null ? "none" : StringUtils.humanReadableInt(sf
						.getReader().length())) + "; total size for store is "
	...
	public void compactRecentForTesting(int N) throws IOException {
		...
		try {
			// Ready to go. Have list of files to compact.
			StoreFile.Writer writer = this.compactor.compactForTesting(this,
					conf, filesToCompact, isMajor, maxId);
			// Move the compaction into place.
	...
	private boolean isMajorCompaction(final List<StoreFile> filesToCompact)
		...
		if (timeToMajorCompaction) {
			...
			if (filesToCompact.size() == 1) {
				...
				long oldest = (sf.getReader().timeRangeTracker == null) ? Long.MIN_VALUE
						: now
								- sf.getReader().timeRangeTracker.minimumTimestamp;
				if (sf.isMajorCompaction()
						&& (this.ttl == HConstants.FOREVER || oldest < this.ttl)) {
					if (LOG.isDebugEnabled()) {
						LOG.debug("Skipping major compaction of "
								+ this
	...
	long getNextMajorCompactTime() {
		...
		if (family.getValue(HConstants.MAJOR_COMPACTION_PERIOD) != null) {
			String strCompactionTime = family
					.getValue(HConstants.MAJOR_COMPACTION_PERIOD);
			ret = (new Long(strCompactionTime)).longValue();
		}

		if (ret > 0) {
			// default = 20% = +/- 4.8 hrs
			double jitterPct = conf.getFloat(
					"hbase.hregion.majorcompaction.jitter", 0.20F);
			if (jitterPct > 0) {
				long jitter = Math.round(ret * jitterPct);
				// deterministic jitter avoids a major compaction storm on
				// restart
	...
	/**
	 * Algorithm to choose which files to compact, see
	 * {@link #compactSelection(java.util.List, int)}
	 * 
	 * @param candidates
	 * @return
	 * @throws IOException
	 ...
	 * 
	 * Configuration knobs: "hbase.hstore.compaction.ratio" normal case: minor
	 * compact when file <= sum(smaller_files) * ratio
	 * "hbase.hstore.compaction.min.size" unconditionally compact individual
	 * files below this size "hbase.hstore.compaction.max.size" never compact
	 * individual files above this size (unless splitting)
	...
	CompactSelection compactSelection(List<StoreFile> candidates, int priority)
		...
		if (!majorcompaction
			 ...
			 * load. Collections.sort(filesToCompact,
			 ...
			 * that meets compaction criteria: (1) a recently-flushed, small
			 * file (i.e. <= minCompactSize) OR (2) within the compactRatio of
			 * sum(newer_files) Given normal skew, any newer files will also
			 * meet this criteria
			 * 
	 ...
	 * 
	 * <p>Moving the compacted TreeMap into place means: <pre> 1) Moving the new
	 * compacted StoreFile into place 2) Unload all replaced StoreFile, close
	 * and collect list to delete. 3) Loading the new TreeMap. 4) Compute new
	 * store size </pre>
	 * 
	...
	StoreFile completeCompaction(final Collection<StoreFile> compactedFiles,
		...
		if (compactedFile != null) {
			...
			LOG.info("Renaming compacted file at " + origPath + " to "
					+ destPath);
			...
			result = new StoreFile(this.fs, destPath, this.conf,
					this.cacheConf, this.family.getBloomFilterType(),
					this.dataBlockEncoder);
		...
		try {
			...
			try {
				...
				// change in case old files are still being accessed by
				// outstanding
				// scanners.
				ArrayList<StoreFile> newStoreFiles = Lists
						.newArrayList(storefiles);
				newStoreFiles.removeAll(compactedFiles);
				filesCompacting.removeAll(compactedFiles); // safe bc:
															// lock.writeLock()
				...
				// or changing the memstore. Let us release it before calling
				// notifyChangeReadersObservers. See HBASE-4485 for a possible
				// deadlock scenario that could have happened if continue to
				// hold
				// the lock.
				this.lock.writeLock().unlock();
			}
			...
			// let the archive util decide if we should archive or delete the
			// files
	 ...
	 * dependent on the axiom that deletes are for cells that are in the
	 * container that follows whether a memstore snapshot or a storefile, not
	 * for the current container: i.e. we'll see deletes before we come across
	 * cells we are to delete. Presumption is that the memstore#kvset is
	 * processed before memstore#snapshot and so on.
	 * 
	 * @param row
	...
	private void rowAtOrBeforeFromStoreFile(final StoreFile f,
		...
		if (this.comparator.compareRows(lastKV, firstOnRow) < 0) {
			// If last key in file is not of the target table, no candidates in
			// this
			// file. Return.
			if (!state.isTargetTable(lastKV))
				return;
			// If the row we're looking for is past the end of file, set search
			// key to
			// last key. TODO: Cache last and first key rather than make each
			// time.
			firstOnRow = new KeyValue(lastKV.getRow(),
		...
		if (!seekToScanner(scanner, firstOnRow, firstKV))
			return;
		// If we found candidate on firstOnRow, just return. THIS WILL NEVER
		// HAPPEN!
		// Unlikely that there'll be an instance of actual first row in table.
		if (walkForwardInSingleRow(scanner, firstOnRow, state))
			return;
		// If here, need to start backing up.
		while (scanner.seekBefore(firstOnRow.getBuffer(),
				firstOnRow.getKeyOffset(), firstOnRow.getKeyLength())) {
			KeyValue kv = scanner.getKeyValue();
	 ...
	 * 
	 * This function will always be seen as atomic by other readers because it
	 * only puts a single KV to memstore. Thus no read/write control necessary.
	 * 
	 * @param row
	 * @param f
	 * @param qualifier
	 ...
	 * @throws IOException
	 */
	public long updateColumnValue(byte[] row, byte[] f, byte[] qualifier,
		...
		try {
			...
			return this.memstore.updateColumnValue(row, f, qualifier, newValue,
					now);
	 ...
	 * qualifier exists in MemStore, it will be replaced. Otherwise, it will
	 * just be inserted to MemStore.
	 * <p>
	...
	private class StoreFlusherImpl implements StoreFlusher {
		...
		public void prepare() {
			...
			this.snapshotTimeRangeTracker = memstore
					.getSnapshotTimeRangeTracker();
		}

		@Override
		public void flushCache(MonitoredTask status) throws IOException {
			storeFilePath = Store.this.flushCache(cacheFlushId, snapshot,
					snapshotTimeRangeTracker, flushedSize, status);
		}
		...
		public boolean commit(MonitoredTask status) throws IOException {
			...
			if (Store.this.getHRegion().getCoprocessorHost() != null) {
				Store.this.getHRegion().getCoprocessorHost()
	...
	public static final long FIXED_OVERHEAD = ClassSize
			.align(SchemaConfigured.SCHEMA_CONFIGURED_UNALIGNED_HEAP_SIZE
	...
	public static class ScanInfo {
		...
		public static final long FIXED_OVERHEAD = ClassSize
				.align(ClassSize.OBJECT + (2 * ClassSize.REFERENCE)
						+ (2 * Bytes.SIZEOF_INT) + Bytes.SIZEOF_LONG
						+ Bytes.SIZEOF_BOOLEAN);

		/**
		 * @param family
		 *            {@link HColumnDescriptor} describing the column family
		 * @param ttl
		 *            Store's TTL (in ms)
		 * @param timeToPurgeDeletes
		 *            duration in ms after which a delete marker can be purged
		 *            during a major compaction.
		 * @param comparator
		 *            The store's comparator
		 */
		public ScanInfo(HColumnDescriptor family, long ttl,
				long timeToPurgeDeletes, KVComparator comparator) {
			this(family.getName(), family.getMinVersions(), family
					.getMaxVersions(), ttl, family.getKeepDeletedCells(),
		 ...
		 *            duration in ms after which a delete marker can be purged
		 *            during a major compaction.
		 * @param keepDeletedCells
		 *            Store's keepDeletedCells setting
		 * @param comparator
		 *            The store's comparator
		 */
		public ScanInfo(byte[] family, int minVersions, int maxVersions,
				long ttl, boolean keepDeletedCells, long timeToPurgeDeletes,
