 ...
 * produced by flushing the memstore to disk. To create, instantiate a writer
 * using {@link StoreFile#WriterBuilder} and append data. Be sure to add any
...
public class StoreFile extends SchemaConfigured {
	...
	public static final byte[] MAJOR_COMPACTION_KEY = Bytes
			.toBytes("MAJOR_COMPACTION_KEY");

	/** Major compaction flag in FileInfo */
	public static final byte[] EXCLUDE_FROM_MINOR_COMPACTION_KEY = Bytes
			.toBytes("EXCLUDE_FROM_MINOR_COMPACTION");

	/** Bloom filter Type in FileInfo */
	public static final byte[] BLOOM_FILTER_TYPE_KEY = Bytes
			.toBytes("BLOOM_FILTER_TYPE");
	...
	private static final byte[] LAST_BLOOM_KEY = Bytes
			.toBytes("LAST_BLOOM_KEY");

	/** Key for Timerange information in metadata */
	public static final byte[] TIMERANGE_KEY = Bytes.toBytes("TIMERANGE");

	/** Key for timestamp of earliest-put in metadata */
	public static final byte[] EARLIEST_PUT_TS = Bytes
			.toBytes("EARLIEST_PUT_TS");
	...
	public static final byte[] BULKLOAD_TASK_KEY = Bytes
			.toBytes("BULKLOAD_SOURCE_TASK");
	public static final byte[] BULKLOAD_TIME_KEY = Bytes
			.toBytes("BULKLOAD_TIMESTAMP");
	 ...
	 * uuid ([0-9a-z]+). Bulk loaded hfiles has (_SeqId_[0-9]+_) has suffix.
	 */
	public static final String HFILE_NAME_REGEX = "[0-9a-f]+(?:_SeqId_[0-9]+_)?";

	/** Regex that will work for hfiles */
	private static final Pattern HFILE_NAME_PATTERN = Pattern.compile("^("
			+ HFILE_NAME_REGEX + ")");

	/**
	 * Regex that will work for straight reference names
	 * (<hfile>.<parentEncRegion>) and hfilelink reference names (
	 * <table>=
	 * <region>-<hfile>.<parentEncRegion>) If reference, then the regex has more
	 * than just one group. Group 1, hfile/hfilelink pattern, is this file's id.
	 * Group 2 '(.+)' is the reference's parent region name.
	 */
	private static final Pattern REF_NAME_PATTERN = Pattern.compile(String
			.format("^(%s|%s)\\.(.+)$", HFILE_NAME_REGEX,
					HFileLink.LINK_NAME_REGEX));
	 ...
	 * @param cfBloomType
	 *            The bloom type to use for this store file as specified by
	 *            column family configuration. This may or may not be the same
	 *            as the Bloom filter type actually present in the HFile,
	 *            because column family configuration might change. If this is
	 ...
	 */
	public StoreFile(final FileSystem fs, final Path p,
		...
		this.dataBlockEncoder = dataBlockEncoder == null ? NoOpDataBlockEncoder.INSTANCE
				: dataBlockEncoder;
	...
	public static Path getReferredToFile(final Path p) {
		...
		if (m == null || !m.matches()) {
			...
			throw new IllegalArgumentException(
					"Failed match of store file name " + p.toString());
		}
		...
		LOG.debug("reference '" + p + "' to region=" + otherRegion + " hfile="
	 ...
	 * list. Store files that were created by a mapreduce bulk load are ignored,
	 * as they do not correspond to any specific put operation, and thus do not
	 * have a memstoreTS associated with them.
	 * 
	 ...
	 * list. Store files that were created by a mapreduce bulk load are ignored,
	 * as they do not correspond to any edit log items.
	 * 
	 * @return 0 if no non-bulk-load files are provided or, this is Store that
	 *         does not yet have any store files.
	 */
	...
	/**
	 * @return true if this storefile was created by HFileOutputFormat for a
	 *         bulk load.
	 */
	 ...
	 * takes the first half of reference file, top part takes the second half of
	 * the reference file. This is just estimate, given midkey ofregion !=
	 * midkey of HFile, also the number and size of keys vary. If this estimate
	 * isn't good enough, we can improve it later.
	 * 
	 * @param fs
	 *            The FileSystem
	 * @param reference
	...
	static private HDFSBlocksDistribution computeRefFileHDFSBlockDistribution(
			FileSystem fs, Reference reference, FileStatus status)
			throws IOException {
		if (status == null) {
			return null;
		}
	...
	private void computeHDFSBlockDistribution() throws IOException {
		if (isReference()) {
			...
			this.hdfsBlocksDistribution = FSUtils
					.computeHDFSBlocksDistribution(this.fs, status, 0, length);
		}
	}
	...
	private Reader open() throws IOException {
		...
		if (isReference()) {
			if (this.link != null) {
				this.reader = new HalfStoreFileReader(this.fs,
						this.referencePath, this.link, this.cacheConf,
			...
			this.reader = new Reader(this.fs, this.path, link, size,
					this.cacheConf, dataBlockEncoder.getEncodingInCache(), true);
		} else {
		...
		if (b != null) {
			...
			// store,
			// since store files are distinguished by sequence id, the one half
			// would
			// subsume the other.
		...
		if (cfBloomType != BloomType.NONE) {
			...
			if (hfileBloomType != cfBloomType) {
				LOG.info("HFile Bloom filter type for "
						...
						+ hfileBloomType + ", but " + cfBloomType
	...
	public static class WriterBuilder {
		 ...
		 * 
		 * @param dir
		 *            Path to column family directory. The directory is created
		 *            if does not exist. The file is given a unique name within
		 *            this directory.
		...
		/**
		 * Create a store file writer. Client is responsible for closing file
		 * when done. If metadata, add BEFORE closing using
		 * {@link Writer#appendMetadata}.
		 */
		public Writer build() throws IOException {
			if ((dir == null ? 0 : 1) + (filePath == null ? 0 : 1) != 1) {
				throw new IllegalArgumentException(
						"Either specify parent directory " + "or file path");
			}
	 ...
	 * package local because it is an implementation detail of the HBase
	 * regionserver.
	 */
	public static class Writer {
		...
		/*
		 * isTimeRangeTrackerSet keeps track if the timeRange has already been
		 * set When flushing a memstore, we set TimeRange and use this variable
		 * to indicate that it doesn't need to be calculated again while
		 * appending KeyValues. It is not set in cases of compactions when it is
		 * recalculated using only the appended KeyValues
		 */
		 ...
		 * 
		 * @param fs
		 *            file system to write to
		 * @param path
		 *            file name to create
		 * @param blocksize
		 *            HDFS block size
		 * @param compress
		 *            HDFS block compression
		 * @param conf
		 *            user configuration
		 * @param comparator
		 *            key comparator
		...
		private Writer(FileSystem fs, Path path, int blocksize,
				...
				final Configuration conf, CacheConfig cacheConf,
				final KVComparator comparator, BloomType bloomType,
				long maxKeys, final ChecksumType checksumType,
			...
			writer = HFile.getWriterFactory(conf, cacheConf).withPath(fs, path)
					.withBlockSize(blocksize).withCompression(compress).withReplication(replication)
					.withDataBlockEncoder(dataBlockEncoder)
					.withComparator(comparator.getRawComparator())
					.withChecksumType(checksumType)
					.withBytesPerChecksum(bytesPerChecksum).create();
			...
			if (generalBloomFilterWriter != null) {
				this.bloomType = bloomType;
				LOG.info("Bloom filter type for " + path + ": "
						+ this.bloomType + ", "
			...
			// initialize delete family Bloom filter when there is NO RowCol
			// Bloom
			// filter
			if (this.bloomType != BloomType.ROWCOL) {
				this.deleteFamilyBloomFilterWriter = BloomFilterFactory
						.createDeleteBloomAtWrite(conf, cacheConf,
								(int) Math.min(maxKeys, Integer.MAX_VALUE),
								writer);
			} else {
				deleteFamilyBloomFilterWriter = null;
			}
			if (deleteFamilyBloomFilterWriter != null) {
				LOG.info("Delete Family Bloom filter type for "
						+ path
						+ ": "
						+ deleteFamilyBloomFilterWriter.getClass()
								.getSimpleName());
			}
			this.checksumType = checksumType;
			this.bytesPerChecksum = bytesPerChecksum;
		 ...
		 * 
		 * If the timeRangeTracker is not set, update TimeRangeTracker to
		 * include the timestamp of this key
		 * 
		 * @param kv
		 */
		...
		private void appendGeneralBloomfilter(final KeyValue kv)
				throws IOException {
				...
				if (this.lastKv != null) {
					...
					default:
						throw new IOException("Invalid Bloom filter type: "
								+ bloomType + " (ROW or ROWCOL expected)");
					}
				}
				if (newKey) {
					/*
					 * http://2.bp.blogspot.com/_Cib_A77V54U/StZMrzaKufI/AAAAAAAAADo
					 * /ZhK7bGoJdMQ/s400/KeyValue.png Key = RowLen + Row +
					 * FamilyLen + Column [Family + Qualifier] + TimeStamp
					 * 
					 * 2 Types of Filtering: 1. Row = Row 2. RowCol = Row +
					 * Qualifier
					 */
					byte[] bloomKey;
					...
					case ROWCOL:
						// merge(row, qualifier)
						// TODO: could save one buffer copy in case of compound
						// Bloom
						// filters when this involves creating a KeyValue
						bloomKey = generalBloomFilterWriter.createBloomKey(
					...
					generalBloomFilterWriter.add(bloomKey, bloomKeyOffset,
							bloomKeyLen);
					if (lastBloomKey != null
							&& generalBloomFilterWriter
									.getComparator()
									.compare(bloomKey, bloomKeyOffset,
											bloomKeyLen, lastBloomKey,
											lastBloomKeyOffset, lastBloomKeyLen) <= 0) {
						throw new IOException("Non-increasing Bloom keys: "
								+ Bytes.toStringBinary(bloomKey,
										bloomKeyOffset, bloomKeyLen)
								+ " after "
								+ Bytes.toStringBinary(lastBloomKey,
										lastBloomKeyOffset, lastBloomKeyLen));
					}
		...
		private boolean closeBloomFilter(BloomFilterWriter bfw)
				throws IOException {
			...
			if (hasGeneralBloom) {
				...
				if (lastBloomKey != null) {
					writer.appendFileInfo(LAST_BLOOM_KEY, Arrays.copyOfRange(
							lastBloomKey, lastBloomKeyOffset,
							lastBloomKeyOffset + lastBloomKeyLen));
				}
			}
		...
		public void close() throws IOException {
			...
			// Log final Bloom filter statistics. This needs to be done after
			// close()
			// because compound Bloom filters might be finalized as part of
			// closing.
			StoreFile.LOG.info((hasGeneralBloom ? "" : "NO ")
					+ "General Bloom and "
	...
	public static class Reader extends SchemaConfigured {
		...
		/**
		 * Get a scanner to scan over this StoreFile. Do not use this overload
		 * if using this scanner for compactions.
		 * 
		 * @param cacheBlocks
		 ...
		 * Instead use getStoreFileScanner() which uses the StoreFileScanner
		 * class/interface which is the preferred way to scan a store with
		 ...
		 *            the oldest timestamp that is not expired, as determined by
		 *            the column family's TTL
		 * @return false if queried keys definitely don't exist in this
		 *         StoreFile
		 */
		 ...
		 * Only checks Bloom filters for single-row or single-row-column scans.
		 * Bloom filter checking for multi-gets is implemented as part of the
		 * store scanner system (see {@link StoreFileScanner#seekExactly}) and
		 ...
		 *         filter, or if the Bloom filter is not applicable for the
		 *         scan. False if the Bloom filter is applicable and the scan
		 *         fails it.
		 */
		boolean passesBloomFilter(Scan scan, final SortedSet<byte[]> columns) {
			...
			case ROWCOL:
				if (columns != null && columns.size() == 1) {
					...
					return passesGeneralBloomFilter(row, 0, row.length, column,
							0, column.length);
				}
		...
		public boolean passesDeleteFamilyBloomFilter(byte[] row, int rowOffset,
				int rowLen) {
			// Cache Bloom filter as a local variable in case it is set to null
			// by
		...
		public boolean passesGeneralBloomFilter(byte[] row, int rowOffset,
			...
			case ROW:
				if (col != null) {
					throw new RuntimeException(
							"Row-only Bloom filter called with "
									+ "column specified");
				}
				if (rowOffset != 0 || rowLen != row.length) {
					throw new AssertionError(
							"For row-only Bloom filters the row "
			...
			case ROWCOL:
				key = generalBloomFilter.createBloomKey(row, rowOffset, rowLen,
						col, colOffset, colLen);
				break;
			...
			try {
				...
				if (bloomFilter.supportsAutoLoading()) {
					...
					bloom = reader.getMetaBlock(
							HFileWriterV1.BLOOM_FILTER_DATA_KEY, true);
				...
				if (shouldCheckBloom) {
					...
					// Whether the primary Bloom key is greater than the last
					// Bloom key
					// from the file info. For row-column Bloom filters this is
					// not yet
					// a sufficient condition to return false.
					boolean keyIsAfterLast = lastBloomKey != null
							&& bloomFilter.getComparator().compare(key,
									lastBloomKey) > 0;

					if (bloomFilterType == BloomType.ROWCOL) {
						// Since a Row Delete is essentially a DeleteFamily
						// applied to all
						// columns, a file might be skipped if using row+col
						// Bloom filter.
						// In order to ensure this file is included an
						// additional check is
						// required looking only for a row bloom.
						byte[] rowBloomKey = bloomFilter.createBloomKey(row, 0,
								row.length, null, 0, 0);

						if (keyIsAfterLast
								&& bloomFilter.getComparator().compare(
										rowBloomKey, lastBloomKey) > 0) {
							exists = false;
						} else {
							exists = bloomFilter.contains(key, 0, key.length,
									bloom)
									|| bloomFilter.contains(rowBloomKey, 0,
											rowBloomKey.length, bloom);
						}
					} else {
						exists = !keyIsAfterLast
								&& bloomFilter.contains(key, 0, key.length,
										bloom);
					}
				...
				if (blockType == BlockType.GENERAL_BLOOM_META) {
					...
					if (bloomMeta != null) {
						...
						if (bloomFilterType == BloomType.NONE) {
							...
							generalBloomFilter = BloomFilterFactory
									.createFromMeta(bloomMeta, reader);
							LOG.info("Loaded "
									+ bloomFilterType.toString()
									+ " ("
									+ generalBloomFilter.getClass()
											.getSimpleName()
					...
					if (bloomMeta != null) {
						deleteFamilyBloomFilter = BloomFilterFactory
								.createFromMeta(bloomMeta, reader);
						LOG.info("Loaded Delete Family Bloom ("
								+ deleteFamilyBloomFilter.getClass()
										.getSimpleName() + ") metadata for "
								+ reader.getName());
					}
				} else {
					throw new RuntimeException("Block Type: "
							+ blockType.toString()
		 ...
		 * thereof, if the Bloom filter is not loaded. This always returns an
		 * upper bound of the number of Bloom filter entries.
		 * 
		 * @return an estimate of the number of Bloom filter entries in this
		 *         file
		 */
		public long getFilterEntries() {
			return generalBloomFilter != null ? generalBloomFilter
					.getKeyCount() : reader.getEntries();
		}
	...
	abstract static class Comparators {
		/**
		 * Comparator that compares based on the flush time of the StoreFiles.
		 * All bulk loads are placed before all non- bulk loads, and then all
		 * files are sorted by sequence ID. If there are ties, the path name is
		 * used as a tie-breaker.
		 */
		static final Comparator<StoreFile> FLUSH_TIME = Ordering
				.compound(ImmutableList.of(
						Ordering.natural().onResultOf(new GetBulkTime()),
						Ordering.natural().onResultOf(new GetSeqId()), Ordering
								.natural().onResultOf(new GetPathName())));
		 ...
		 */
		static final Comparator<StoreFile> FILE_SIZE = Ordering.natural()
				.reverse().onResultOf(new Function<StoreFile, Long>() {
					@Override
					public Long apply(StoreFile sf) {
