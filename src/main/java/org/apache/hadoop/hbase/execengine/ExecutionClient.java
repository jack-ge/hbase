...
public class ExecutionClient {
	...
	public List<EvaluationResult[]> getTopNByRow(byte[] tableName,
		...
		validateAndDecorateParameters(scans, new ArrayList(), selectExpressions);

		ClientCallback<List<EvaluationResult[]>> topNCB = new BatchExecCall.ClientCallback<List<EvaluationResult[]>>() {
		...
		for (final Scan scan : scans) {
			calls.add(new Pair(
					new Pair(scan.getStartRow(), scan.getStopRow()),
	...
	protected Map<GroupByCombinedKey, List<StatsValue>> getTopNInternal(
			...
			throws Throwable {
		final Map map = requireSortedMap ? new TreeMap(comparator)
				: new HashMap();
		...
		class TopNCallback implements
			...
			public synchronized void update(ServerName server,
				...
				for (Map.Entry entry : result.getStatsMap().entrySet()) {
					List finalStats = (List) map.get(entry.getKey());
					Iterator itFinal;
					Iterator itTemp;
					if (finalStats != null) {
						itFinal = finalStats.iterator();
						for (itTemp = ((List) entry.getValue()).iterator(); itFinal
								.hasNext();)
							try {
		...
		class RowNumCallback
			...
			public synchronized void update(ServerName server,
				...
				for (GroupByCombinedKey facet : resultMap.keySet()) {
					List finalStats = (List) this.finalMap.get(facet);
					Iterator itFinal;
					Iterator itTemp;
					if (finalStats != null) {
						itFinal = finalStats.iterator();
						for (itTemp = ((List) resultMap.get(facet)).iterator(); itFinal
								.hasNext();)
							try {
								((StatsValue) itFinal.next())
										.accumulate((StatsValue) itTemp.next());
	...
	private boolean validateAndDecorateParameters(Scan[] scans,
		...
		for (Expression keyExpr : groupByKeyExpressions) {
			...
			for (Pair columnPair : visitor.getColumnSet()) {
