...
public class CacheConfig {
	...
	/**
	 * TODO: Implement this (jgray) Configuration key to cache data blocks in
	 * compressed format.
	 */
	public static final String CACHE_DATA_BLOCKS_COMPRESSED_KEY = "hbase.rs.blockcache.cachedatacompressed";

	/**
	 * Configuration key to evict all blocks of a given file from the block
	 * cache when the file is closed.
	 */
	...
	public CacheConfig(Configuration conf, HColumnDescriptor family) {
		...
		// if they are enabled in the global configuration.
				conf.getBoolean(CACHE_BLOCKS_ON_WRITE_KEY,
						...
						|| family.shouldCacheDataOnWrite(), conf.getBoolean(
						CACHE_INDEX_BLOCKS_ON_WRITE_KEY,
						DEFAULT_CACHE_INDEXES_ON_WRITE)
						|| family.shouldCacheIndexesOnWrite(), conf.getBoolean(
						CACHE_BLOOM_BLOCKS_ON_WRITE_KEY,
						DEFAULT_CACHE_BLOOMS_ON_WRITE)
						|| family.shouldCacheBloomsOnWrite(), conf.getBoolean(
						EVICT_BLOCKS_ON_CLOSE_KEY, DEFAULT_EVICT_ON_CLOSE)
						|| family.shouldEvictBlocksOnClose(), conf.getBoolean(
	...
	public CacheConfig(Configuration conf) {
		this(CacheConfig.instantiateBlockCache(conf),CacheConfig.instantiateBlobFileCache(conf),
				DEFAULT_CACHE_DATA_ON_READ, DEFAULT_IN_MEMORY, // This is a
																...
																// conf
				conf.getBoolean(CACHE_BLOCKS_ON_WRITE_KEY,
						DEFAULT_CACHE_DATA_ON_WRITE), conf.getBoolean(
						CACHE_INDEX_BLOCKS_ON_WRITE_KEY,
						DEFAULT_CACHE_INDEXES_ON_WRITE), conf.getBoolean(
						CACHE_BLOOM_BLOCKS_ON_WRITE_KEY,
						DEFAULT_CACHE_BLOOMS_ON_WRITE), conf.getBoolean(
						EVICT_BLOCKS_ON_CLOSE_KEY, DEFAULT_EVICT_ON_CLOSE),
	 ...
	 * 
	 * @param blockCache
	 *            reference to block cache, null if completely disabled
	 * @param cacheDataOnRead
	 *            whether data blocks should be cached on read
	 * @param inMemory
	 *            whether blocks should be flagged as in-memory
	 * @param cacheDataOnWrite
	 *            whether data blocks should be cached on write
	 * @param cacheIndexesOnWrite
	 *            whether index blocks should be cached on write
	 * @param cacheBloomsOnWrite
	 *            whether blooms should be cached on write
	 * @param evictOnClose
	 *            whether blocks should be evicted when HFile is closed
	 * @param cacheCompressed
	 *            whether to store blocks as compressed in the cache
	 */
	CacheConfig(BlockCache blockCache, boolean cacheDataOnRead,
			boolean inMemory, boolean cacheDataOnWrite,
			boolean cacheIndexesOnWrite, boolean cacheBloomsOnWrite,
			boolean evictOnClose, boolean cacheCompressed) {
		this(blockCache, null, cacheDataOnRead, inMemory, cacheDataOnWrite,
				cacheIndexesOnWrite, cacheBloomsOnWrite, evictOnClose,
				cacheCompressed);
	}
	
	CacheConfig(BlockCache blockCache, BlobFileCache blobFileCache,
			boolean cacheDataOnRead, boolean inMemory,
			boolean cacheDataOnWrite, boolean cacheIndexesOnWrite,
			boolean cacheBloomsOnWrite, boolean evictOnClose,
			boolean cacheCompressed) {
	...
	public CacheConfig(CacheConfig cacheConf) {
		this(cacheConf.blockCache,cacheConf.blobFileCache, cacheConf.cacheDataOnRead,
				cacheConf.inMemory, cacheConf.cacheDataOnWrite,
				cacheConf.cacheIndexesOnWrite, cacheConf.cacheBloomsOnWrite,
				cacheConf.evictOnClose, cacheConf.cacheCompressed);
	}
	...
	/**
	 * @return true if data blocks should be written to the cache when an HFile
	 ...
	 * 
	 * @param cacheDataOnWrite
	 *            whether data blocks should be written to the cache when an
	 *            HFile is written
	 */
	 ...
	 * 
	 * @param evictOnClose
	 *            whether blocks should be evicted from the cache when an HFile
	 *            reader is closed
	 */
	...
	public String toString() {
		...
		return "CacheConfig:enabled " + "[cacheDataOnRead="
				+ shouldCacheDataOnRead() + "] " + "[cacheDataOnWrite="
				+ shouldCacheDataOnWrite() + "] " + "[cacheIndexesOnWrite="
				+ shouldCacheIndexesOnWrite() + "] " + "[cacheBloomsOnWrite="
				+ shouldCacheBloomsOnWrite() + "] " + "[cacheEvictOnClose="
				+ shouldEvictOnClose() + "] " + "[cacheCompressed="
	...
	private static synchronized BlockCache instantiateBlockCache(
		...
		long offHeapCacheSize = (long) (conf.getFloat(
				"hbase.offheapcache.percentage", (float) 0) * DirectMemoryUtils
				.getDirectMemorySize());
		...
		if (offHeapCacheSize <= 0) {
			...
			globalBlockCache = new DoubleBlockCache(cacheSize,
					offHeapCacheSize, StoreFile.DEFAULT_BLOCKSIZE_SMALL,
					blockSize, conf);
		}
		return globalBlockCache;
	}
